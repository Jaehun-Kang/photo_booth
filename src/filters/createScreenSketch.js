...
p5js.setup = () => {
  ...
  const h = window.innerHeight;        // 창 높이
  const w = h * videoAspect;            // 비율 맞춘 너비
...
p5js.draw = () => {
  ...
  if (filter.draw) {
    try {
      ...
      console.warn('filter.draw error:', e);
    }
  } else {
    p5js.image(offscreen, offsetX, offsetY, drawWidth, drawHeight);
  }
};
