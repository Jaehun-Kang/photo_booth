  ...
  return function sketch(p5js) {
    ...
    p5js.draw = () => {
      ...
      if (lastVideoSize !== currentVideoSize) {
        console.log(`🔄 비디오 해상도 변화 감지: ${lastVideoSize || 'null'} -> ${currentVideoSize}`);
      ...
      if (!debugLogged) {
        ...
        console.log(`Video: ${video.videoWidth}x${video.videoHeight} (비율: ${videoRatio.toFixed(3)})`);
        console.log(`Target: 3:2 (${targetRatio.toFixed(3)})`);
        console.log(`크롭핑 방식: ${cropMethod}`);
        console.log(`크롭 영역: sx:${sx}, sy:${sy}, sw:${sw.toFixed(1)}, sh:${sh.toFixed(1)}`);
        console.log(`여백 - 좌우: ${sx}, ${(video.videoWidth - sx - sw).toFixed(1)} / 상하: ${sy}, ${(video.videoHeight - sy - sh).toFixed(1)}`);
        
        const isValidCrop = sx >= 0 && sy >= 0 && (sx + sw) <= video.videoWidth && (sy + sh) <= video.videoHeight;
